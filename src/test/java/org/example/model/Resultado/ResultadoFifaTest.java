...
class ResultadoFifaTest {
    ...
    void testGanadorLocal() {
        ResultadoFifa resultado = new ResultadoFifa(local, visitante, 3, 1, local);
    ...
    void testGanadorVisitante() {
        ResultadoFifa resultado = new ResultadoFifa(local, visitante, 0, 2, visitante);
        assertTrue(resultado.esValido());
    ...
    @Test
    void testEmpate() {
        ResultadoFifa resultado = new ResultadoFifa(local, visitante, 2, 2, null);
        assertTrue(resultado.esValido());
    ...
    void testGolesNegativosLanzaExcepcion() {
        assertThrows(IllegalArgumentException.class, () -> new ResultadoFifa(local, visitante, -1, 2, null));
    }
}
